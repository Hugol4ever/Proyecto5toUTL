package com.example.ddd_market.ddd_market.commons;

import java.text.SimpleDateFormat;

/**
 * Created by hugo_ on 06/04/2017.
 */

public class Globals {

    public static String SERVIDOR = "192.168.1.7";//computadora rosa
    public static int VERSION_DB = 6;
    public static String NOMBRE_DB = "DDD";
    public static String SERVIDOR_IMAGENES = "http://192.168.1.11:8080/FotosWebService5to/";//computadora negra
    public static SimpleDateFormat FECHA = new SimpleDateFormat("yyyy-MM-dd");
    public static SimpleDateFormat HORA = new SimpleDateFormat("HH:mm:ss");

}